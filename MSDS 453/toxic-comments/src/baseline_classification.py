"""Train a simple model on cleaned data as a baseline."""
...
class comment_classifier:
    ...
    def full_fit(self, target):
        ...
        # Specify the model
        # For logistic regression on large data, the saga solver is faster
        # Since there is a large number of features, regularization is needed
        # L1 is LASSO which has the advantage of reducing weight to zero
...
train = pd.read_csv(data_directory / 'train_processed.csv').fillna(" ")
test = pd.read_csv(data_directory / 'test_processed.csv').fillna(" ")
...
tfidf_vect = TfidfVectorizer(max_features=1000, sublinear_tf=True)
...
# Persist TF-IDF data
train_tfidf.to_csv(data_directory / 'train_tfidf.csv')
test_tfidf.to_csv(data_directory / 'test_tfidf.csv')

# Specify output directories
plot_directory = Path('./reports/figures/baseline')
...
# Place to store cross validation results
cv_results = {}
...
for y in y_names:
    ...
    # Generate cross-validation results
    cv_results[y] = baseline_model.cross_val()
...
# Store CV results
with open(output_directory / 'baseline_cv.txt', 'w') as f:
    print("Cross-Validation AUC", file=f)
    for i in cv_results:
        print(f"Class: {i}, AUC: {cv_results[i]:.6f}", file=f)

# Save entire submission
submission.to_csv(submissions_directory / 'base_submission.csv', index=False)
