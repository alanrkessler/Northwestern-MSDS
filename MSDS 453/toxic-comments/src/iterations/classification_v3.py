"""Train a model with additional TF-IDF ngrams - V3."""
...
# Specify vectorizer using single to three word terms for baseline.
...
tfidf_vect = TfidfVectorizer(max_features=3000, ngram_range=(1, 3),
                             sublinear_tf=True)
...
# Persist TF-IDF data
# train_tfidf.to_csv(data_directory / 'train_order_tfidf.csv')
# test_tfidf.to_csv(data_directory / 'test_order_tfidf.csv')

# Specify output directories
plot_directory = Path('./reports/figures/v3')
...
# Store CV results
with open(output_directory / 'v3_cv.txt', 'w') as f:
    print("Cross-Validation AUC", file=f)
    for i in cv_results:
        print(f"Class: {i}, AUC: {cv_results[i]:.6f}", file=f)

# Save entire submission
submission.to_csv(submissions_directory / 'v3_submission.csv', index=False)
