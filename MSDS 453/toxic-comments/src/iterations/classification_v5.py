"""Train a model based on V4 that uses a low C value."""
...
plot_directory = Path('./reports/figures/v5')
...
# Store CV results
with open(output_directory / 'v5_cv.txt', 'w') as f:
    print("Cross-Validation AUC", file=f)
    for i in cv_results:
        print(f"Class: {i}, AUC: {cv_results[i]:.6f}", file=f)

# Save entire submission
submission.to_csv(submissions_directory / 'v5_submission.csv', index=False)
