"""Train a model on reordered processing - V1."""
...
plot_directory = Path('./reports/figures/v1')
...
# Store CV results
with open(output_directory / 'v1_cv.txt', 'w') as f:
    print("Cross-Validation AUC", file=f)
    for i in cv_results:
        print(f"Class: {i}, AUC: {cv_results[i]:.6f}", file=f)

# Save entire submission
submission.to_csv(submissions_directory / 'v1_submission.csv', index=False)
