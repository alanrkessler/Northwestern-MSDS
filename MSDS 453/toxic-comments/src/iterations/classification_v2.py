"""Train a model with additional TF-IDF features - V2."""
...
plot_directory = Path('./reports/figures/v2')
...
# Store CV results
with open(output_directory / 'v2_cv.txt', 'w') as f:
    print("Cross-Validation AUC", file=f)
    for i in cv_results:
        print(f"Class: {i}, AUC: {cv_results[i]:.6f}", file=f)

# Save entire submission
submission.to_csv(submissions_directory / 'v2_submission.csv', index=False)
