"""Train a model based on V2 that uses l2 regularization."""
...
class comment_classifier:
    ...
    def full_fit(self, target):
        ...
        model = LogisticRegression(solver='sag', C=1)
...
plot_directory = Path('./reports/figures/v4')
...
# Store CV results
with open(output_directory / 'v4_cv.txt', 'w') as f:
    print("Cross-Validation AUC", file=f)
    for i in cv_results:
        print(f"Class: {i}, AUC: {cv_results[i]:.6f}", file=f)

# Save entire submission
submission.to_csv(submissions_directory / 'v4_submission.csv', index=False)
