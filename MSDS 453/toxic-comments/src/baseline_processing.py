"""Apply comment processing to large amounts of data."""
...
train_dd.compute().to_csv(data_directory / 'train_processed.csv')
end = timer()

# Save processing time for training data
output_directory = Path('./reports/output')
with open(output_directory / 'full_processing.txt', 'w') as f:
...
test_dd.compute().to_csv(data_directory / 'test_processed.csv')
end = timer()

# Save processing time
with open(output_directory / 'full_processing.txt', 'a') as f:
